...
shopController.createShop = ('/create-shop', async (req, res)=>{
    ...
    try{
        ...
        if(utilities.createShopValidator(payload, ["name", "shopAddress"]).isValid){
            ...
            if(!verificationDocs){
                utilities.setResponseData(res, 401, {'content-type': 'application/json'}, {statusCode: 401, msg: `Please verify your account by uploading a picture of your ID card and utility bill`, timartToken: newToken}, true )
                return
            }

            if(!verificationDocs.verified){
                utilities.setResponseData(res, 401, {'content-type': 'application/json'}, {statusCode: 401, msg: `Your verification is pending please be patient`, timartToken: newToken}, true )
                return
            }
            ...
            if(!doesUsernameExist){
                ...
                utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, shopData: shopObj, timartToken: newToken}, true )
            }
            else{
               
                utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `You have a shop with this name already, try giving your new shop a name you haven't given any other shop`, timartToken: newToken}, true )
                return
            }
            ...
            //create newToken
            const newToken = utilities.jwt('sign', {userID: decodedToken.userID, tokenFor: "trader"})
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: 'Invalid data, all data should be in string format', timartToken: newToken}, true )
            return
        }
    ...
    try{
        ...
        if(utilities.validator(payload, ["name", "shopAddress", "shopID"]).isValid){
            ...
            if(shopObj?.owner.toString() == decodedToken.userID){
                ...
                //create newToken
                const newToken = utilities.jwt('sign', {userID: decodedToken.userID, tokenFor: "trader"})
                utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `this shop doesn't belong to trader`, timartToken: newToken}, true )
                return
            }  
        }
        else{
...
shopController.getShop = ('/get-shop', async (req, res)=>{
    ...
    try{
        ...
        if(shopObj && shopObj.owner.toString() === decodedToken.userID){
            ...
            //send newToken
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `this shop doesn't belong to trader or this shop doesn't exist`, timartToken: newToken}, true )
            return
        }
    ...
    try{
        ...
        if(utilities.validator(payload, ["shopID"]).isValid){
            ...
            if(shopObj){
                ...
                utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `this shop doesn't exist`, timartToken: newToken}, true )
                return
            }  
        }
        else{
    ...
    try{
        ...
        if(utilities.validator({shopID: shopID}, ["shopID"]).isValid){
            ...
            if(shopObj){
                ...
                //send token
                utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, msg: 'sucess', timartToken: newToken}, true)
            }
            else{
