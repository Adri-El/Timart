...
buyerControllerDashboard.home = ('/dashboard', async (req, res)=>{
  ...
  try{
    ...
    if(buyerObj){
      ...
      utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, buyerData: buyerObj, timartToken: newToken}, true )
    }
    else{
      //send token 
    ...
    if(utilities.validator(payload, ['email', 'password']).isValid){
      ...
      if(payload.password === buyerObj.password){
        ...
        if(!(searchResult.doesUserDetailExist)){
          ...
          await email.subSend('timartltd@gmail.com', payload.email, `hello ${buyerObj.firstName} ${buyerObj.lastName}, please verify your email with this OTP: ${newOtp}`, "OTP Verification", decodedToken.userID)
  ...
  try{
    ...
    if(utilities.validator(payload, ['oldPassword', 'newPassword']).isValid){
      ...
      if(payload.oldPassword === buyerObj.password){
        ...
        await email.subSend('timartltd@gmail.com', buyerObj.email, `hello ${buyerObj.firstName} ${buyerObj.lastName}, please verify your email with this OTP: ${newOtp}`, "OTP Verification", decodedToken.userID)
