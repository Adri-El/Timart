...
productController.addProduct = ('/add-product', async (req, res)=>{
    ...
    try{
        ...
        if(shopObj?.owner.toString() != decodedToken.userID){
            //send new token
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This shop doesn't belong to the trader`, timartToken: newToken}, true)
            return
        }
        ...
        if(utilities.addProductValidator(req.body, ['name', 'price', 'description', 'stock', "category", "weight"]).isValid){
            ...
            // send new token
            utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, productData: productObj, timartToken: newToken}, true)
           
             
        }else{

            //send new token
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, errorObj: utilities.addProductValidator(req.body, ['name', 'price', 'description', 'stock', "category", "weight"]), timartToken: newToken}, true)
            return
...
productController.updateProduct = ('/update-product', async(req, res)=>{
    ...
    try{
        ...
        if(!productObj){
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: "This product does not exist ", timartToken: newToken}, true)
            return;
        } 
        //validate payload
        if(utilities.addProductValidator(req.body, ['name', 'price', 'description', 'stock', "category", "weight"]).isValid){
            ...
            //send new token
            utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, updatedProductData: updatedProduct, timartToken: newToken}, true)

        }
        else{
            
            //send new token
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: utilities.addProductValidator(req.body, ['name', 'price', 'description', 'stock', "category", "weight"]).msg, timartToken: newToken}, true)
        }
...
productController.deleteProduct = ('/delete-product', async (req, res)=>{
    ...
    try{
        if(!(req.query.productID)) return utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `Invalid product ID`, timartToken: newToken}, true) 
        ...
        if(productObj?.owner.toString() === decodedToken.userID){
            ...
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This product does not belong to this trader`, timartToken: newToken}, true)
        }
