...
traderControllerDashboard.home = ('/dashboard', async (req, res)=>{
  ...
  try{
    ...
    if(traderObj){
      ...
      utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, traderData: traderObj, timartToken: newToken}, true )
    }
    else{
      //create token 
      const newToken = utilities.jwt('sign', {userID: decodedToken.userID, tokenFor: decodedToken.tokenFor})
      utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This user doesn't exist`, timartToken: newToken}, true )
      return
    }
  ...
  try{
    ...
    if(utilities.traderProfileUpdateValidator(payload, ['firstName', 'lastName', 'username', 'phoneNumber']).isValid){
      ...
      if(errorArray.length < 1){
        ...
        utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This ${errorArray[0].userDetail} already exists`, timartToken: newToken}, true )
        return
      }
    }
    else{
      utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, errorObj: utilities.traderProfileUpdateValidator(payload, ['firstName', 'lastName', 'username', 'phoneNumber']), timartToken: newToken}, true )
      return
    }
...
traderControllerDashboard.updateBankDetails = ('/update-bank-details', async (req, res)=>{
  ...
  try{
    ...
    await email.sendOtp('timartltd@gmail.com', traderObj.email, "OTP verification", `hello ${traderObj.firstName} ${traderObj.lastName}, please verify your update with this OTP:`, newOtp)
...
traderControllerDashboard.updateEmail = ('/update-email', async (req, res)=>{
  ...
  try{
    ...
    if(!emailDoc){
      utilities.setResponseData(res, 201, {'content-type': 'application/json'}, {statusCode: 201, msg: `You need to subscribe to our email services to recieve OTP`, timartToken: newToken}, true )
      return 
    }
    ...
    if(utilities.validator(payload, ['email', 'password']).isValid){
      ...
      if(payload.password === traderObj.password){
        ...
        if(!(searchResult.doesUserDetailExist)){
          ...
          //send the new otp to the new email
          await email.subSend('timartltd@gmail.com', payload.email, `hello ${traderObj.firstName} ${traderObj.lastName}, please verify your email with this OTP: ${newOtp}`, "OTP Verification", decodedToken.userID)
          ...
          utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This email already exists`, timartToken: newToken}, true )
          return
        }
  ...
  try{
    ...
    if(utilities.validator(payload, ['oldPassword', 'newPassword']).isValid){
      ...
      if(payload.oldPassword === traderObj.password){
        ...
        await email.subSend('timartltd@gmail.com', traderObj.email, `hello ${traderObj.firstName} ${traderObj.lastName}, please verify your email with this OTP: ${newOtp}`, "OTP Verification", decodedToken.userID)
    ...
    if(utilities.confirmBankDetailsValidator(payload, ["amount"]).isValid){
      ...
      if(traderObj.bankDetails){
        ...
        if(traderObj.accountBalance > 0 && traderObj.accountBalance >= payload.amount){
          ...
          await email.sendWithdrawalSummary("timartltd@gmail.com", traderObj.email, "Withdrawal Summary", msg)
