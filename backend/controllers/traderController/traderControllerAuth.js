  ...
  try{
    ...
    if(utilities.signupValidator(payload, ['firstName', 'lastName', 'email', 'username', 'phoneNumber', 'password']).isValid){
      ...
      if(existingUser.constructor.name == 'Object' && !existingUser.doesUserDetailExist){
        ...
        await email.sendOtp('timartltd@gmail.com', payload.email, "OTP Verification", `hello ${payload.firstName} ${payload.lastName}, please verify your email with this OTP:`, payload.otp)
...
traderControllerAuth.verifyOtp = ('/signup/account-verification', async (req, res)=>{
  ...
  try{ 
    ...
    if(utilities.validator(payload, ["otp"]).isValid){
      ...
      //create token 
      const newToken = utilities.jwt('sign', {userID: decodedToken.userID, tokenFor: decodedToken.tokenFor})
      utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `OTP should be a string`, timartToken: newToken}, true )
      return
    }     
  }
  catch(err){
...
traderControllerAuth.resendOtp = ('/signup/resend-otp', async (req, res)=>{
  ...
  try{
    ...
    await email.sendOtp('timartltd@gmail.com', pendingTraderObj.email, "OTP Verification", `hello ${pendingTraderObj.firstName} ${pendingTraderObj.lastName}, please verify your email with this OTP:`, newOtp)
  ...
  try{
    ...
    if(utilities.validator(payload, ['email', 'newPassword']).isValid){
      ...
      if(traderObj){
        ...
        await email.sendOtp('timartltd@gmail.com', payload.email, "OTP Verification", `hello ${traderObj.firstName} ${traderObj.lastName}, please verify your email with this OTP:`, newOtp)
