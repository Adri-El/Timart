        ...
        if(utilities.validator(payload, ['text', 'productID']).isValid){
            ...
            if(productObj){
               ...
               await send("comment", {...commentObj}, commentObj.owner.primaryID, productObj.owner)
                ...
                //send new token
                utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, commentData: commentObj, timartToken: newToken}, true)
    ...
    try{
        ...
        if(utilities.validator(payload, ['text']).isValid){
            ...
            if(commentObj?.owner.toString() === decodedToken.userID.toString()){
                ...
                //send new token
                utilities.setResponseData(res, 200, {'content-type': 'application/json'}, {statusCode: 200, commentData: updatedCommentObj, timartToken: newToken}, true)

            }
            else{
                //send new token
                utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `This user doesn't own this comment`, timartToken: newToken}, true)  
            }
