...
middleware.isJSON = (req, res, next)=>{
    ...
    if(utilities.isJSON(req.body)){
        ...
        if(decodedToken){
            ...
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `Invalid format, payload should be in JSON format`, timartToken: newToken}, true )
        }
        else{
...
middleware.multipartFormParser = async(req, res, next)=>{
    ...
    upload.any()(req, res, (err) => {
        if (err) {
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: err.message, timartToken: newToken}, true )
            return ;
        }
        next();
    });
...
middleware.isTrader = async(req, res, next)=>{
    ...
    if(decodedToken.tokenFor === 'trader'){
        ...
        utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `Only traders can perform this task`, timartToken: newToken}, true ) 
    }
...
middleware.isAdmin = async(req, res, next)=>{
    ...
    if(decodedToken.tokenFor === 'admin'){
        ...
        if(adminObj){
            ...
            utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `invalid ID`, timartToken: newToken}, true ) 
        }
    }
    else{
        utilities.setResponseData(res, 400, {'content-type': 'application/json'}, {statusCode: 400, msg: `You are not authorised to perform this task`, timartToken: newToken}, true ) 
    }
}
